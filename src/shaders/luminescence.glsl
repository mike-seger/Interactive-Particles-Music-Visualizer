...
// Audio reactivity (single-pass): iChannel0 is our 512x2 audio texture.
// We sample the FFT row (y ~= 0.25). Value is in [0,1].
float gAudio = 0.0;
...
float audioLevel() {
    float a = 0.0;
    // A few bins across the spectrum; keep it cheap and stable.
    for (int i = 0; i < 8; i++) {
        float x = (float(i) + 0.5) / 8.0;
        a += sampleFFT(x);
    }
    a /= 8.0;
    // Bias for nicer response on typical music (more punch, less noise floor).
    return clamp(pow(a, 1.6), 0.0, 1.0);
}

float audioForSeed(float seed) {
    // Map each jellyfish to a couple of different FFT bins so they react differently.
    float x1 = fract(seed * 7.13 + 0.11);
    float x2 = fract(seed * 13.71 + 0.37);
    float a = 0.65 * sampleFFT(x1) + 0.35 * sampleFFT(x2);
    // Shape similarly to audioLevel(), but keep a bit more linear variation.
    return clamp(pow(a, 1.25), 0.0, 1.0);
}
...
de map( vec3 p, vec3 id ) {
    ...
    // Per-jellyfish audio (desynced across individuals).
    float aLocal = mix(gAudio, audioForSeed(N), 0.85);

    // Audio-reactive overall size (subtle pulsating, varied per jellyfish).
    float audioPulse = 1.0 + aLocal * (0.06 + 0.06 * sin(iTime * 6.0 + N * twopi));
    float r = 1.0 * audioPulse;
    
    float pump = cos(x+cos(x))+sin(2.*x)*.2+sin(4.*x)*.02;
    
    x = t + N*twopi;
    p.y -= (cos(x+cos(x))+sin(2.*x)*.2)*.6;
    // Make the "breathing" slightly more pronounced with per-jellyfish audio.
    p.xz *= 1. + pump * (0.2 + aLocal * 0.10);
...
de CastRay(ray r) {
    ...
    for(float i=0.; i<MAX_STEPS; i++) {
        ...
        s = map(p, vec3(0.));
        #else
        // Base drift through the field.
        // Vary per-jellyfish ascent speed by using the (time-shifted) cell id as a stable seed.
        vec3 pShift = p;
        pShift.x += t;
        pShift.y -= t;
        rc qSeed = Repeat(pShift, grid);
        float Nc = N3(qSeed.id);
        float aLocal = mix(gAudio, audioForSeed(Nc), 0.85);
        float baseVar = 0.85 + 0.30 * Nc; // slight variety even without audio
        float speed = baseVar + aLocal * 0.22 * (0.2 + 0.8 * Nc);
        pShift.y = p.y - t * speed;
        q = Repeat(pShift, grid);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv.y *= iResolution.y/iResolution.x; 
        ...
        // No mouse control: always auto-camera.
        // (Compute audio here so all downstream functions can use gAudio.)
        gAudio = audioLevel();
        vec2 m = vec2(t * 0.25, SIN(t * pi) * 0.5 + 0.5);
    ...
    // Very subtle audio tilt.
    float camBob = (gAudio - 0.2) * 0.08;
    vec3 camPos = vec3(0., INVERTMOUSE * camDist * cos((m.y + camBob) * pi), camDist) * rotX;

...
// Based on https://github.com/Jxl-s/webgl-blob
// https://webgl-blob.vercel.app/

export default class WebGLBlob extends THREE.Object3D {
  constructor() {
    super()
    this.name = 'WebGLBlob'
  }
  ...
  init() {
    ...
    this.uniforms = {
      ...
      uSpeed: { value: 1.0 },
      uLowColor: { value: new THREE.Color('#89f2ff') },
      uHighColor: { value: new THREE.Color('#0ae2ff') }
    }
    ...
    // Create sphere with high subdivision
    const geometry = new THREE.SphereGeometry(4, 128, 128)
    this.mesh = new THREE.Mesh(geometry, material)
    this.add(this.mesh)

    // Add wireframe ground plane
  ...
  update(deltaTime) {
    ...
    if (!audioManager.isPlaying && !audioManager.isUsingMicrophone) {
      // Reset amplitude when no audio
      this.uniforms.uAmplitude.value = 1.5
      return
    }
    ...
    // Use average of first frequency bin for amplitude modulation
    const avgFrequency = dataArray[0] / 255

    // Modulate amplitude based on audio (1.5 base + up to 5 from audio)
    this.uniforms.uAmplitude.value = 1.5 + (avgFrequency * 5)
  }
